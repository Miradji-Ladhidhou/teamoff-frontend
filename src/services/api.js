...
api.interceptors.response.use(
  (response) => {
    if (shouldNotify(response.config)) {
      ...
      if (message) toastService.add(normalizeNotificationMessage(message, 'success'), 'success', 4000);
    }
    return response;
  },
  async (error) => {
...
export const authService = {
  ...
  setOwnDelegate: (delegue_id) => api.put('/me/delegate', { delegue_id }),
...
export const settingsService = {
  ...
  getDriveBackups: () => api.get('/settings/drive-backups'),
  runRestoreDrive: (fileId, filename) => api.post('/settings/actions/restore-drive', { fileId, filename }),
